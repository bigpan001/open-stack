...
Dispatch all explorers with parallel `spawn_agent` calls, then wait for every result:

- `task_name`: a short unique identifier
- `model`: the configured `how-explorer` route when available; otherwise inherit the parent
- Brief: read-only analysis; do not modify files or external state
...
After the explanation is complete, spawn one architectural critic per available route in `how-critics`. If no panel is configured, run two independent read-only critics that inherit the parent model.
