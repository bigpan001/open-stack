...
6. **Verify the chain from artifacts, not self-report.** Inspect each candidate's output, tool evidence available in the current task, and the files it actually changed. Do not scan filesystem transcripts or unrelated tasks. Grade chain-following from evidence and code shape, never from the candidate's claims alone.
