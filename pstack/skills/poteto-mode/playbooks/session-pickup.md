...
1. Locate the prior trail through an in-scope Codex task, a user-supplied shared link, or a pushed branch. Use Codex thread tools when available; do not scan filesystem transcripts or unrelated tasks. Read the overview and latest turns first, then scan back for decision points. Reduce long history in a read-only sub-agent when delegation is authorized.
