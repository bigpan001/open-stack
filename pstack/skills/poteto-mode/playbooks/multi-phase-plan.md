...
3. Explore in subagents with a brief that explicitly invokes `$poteto-agent` and an explicit model per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
**Control surface.** Pick the available tool by surface. Use browser or native computer control for web and Electron, terminal control for CLIs and TUIs, and the repository's simulator path for mobile. A PR that touches two surfaces gets lanes on both. A surface with no usable control path is a risk in Appendix C.
...
- [ ] Arm the 30-minute audit tick. In a local session, a real terminal a background loop or heartbeat automation. In a cloud root, a cloud-sleeper wake chain. Never leave the cadence to memory.
...
Each live lane runs from an isolated worktree or environment at the PR head. Drive it through the available browser, native computer, terminal, or simulator tool.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes on `gpt-5.6-luna` at the PR head, per the boot recipe.
