...
**PRs.** Use an installed `deslop` skill when available; otherwise review the diff directly. Run `$no-comments` before review. Write every PR title, PR description, and commit body with `$technical-writing`, then apply `$unslop`.
...
- `## Verification`. State how you ran each check and its rigor. Name the real browser, terminal, simulator, or targeted-test path. State the outcome, not only the command name.
