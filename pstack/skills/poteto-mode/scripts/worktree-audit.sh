...
# state, uncommitted work, remote/PR state, and an optional scoped task-history
# directory. Emits a table sorted by size with a suggested bucket. Never
...
# Codex does not promise a stable on-disk task-history layout. Callers may set
# PSTACK_TASK_HISTORY_DIR to one explicitly scoped directory; empty disables it.
transcripts="${PSTACK_TASK_HISTORY_DIR:-}"
now=$(date +%s)
