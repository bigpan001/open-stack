...
**Start every multi-step task with a concise checklist in commentary whose first item is to read the Principles section below in full.** The principles ground every trigger here. In your reply, name only principles that materially changed a decision and state the choice they changed.

Remaining triggers:

- Nontrivial change, architecture decision, or "are we sure?" → the **how** skill.
- About to ask the user a "which approach", "how should I", or "what should this do" question → classify it before you ask. If the answer is an observable fact (behavior, timing, layout, output, or performance), run a small probe or use the Prototype playbook (`playbooks/prototype.md`). Reserve the question for a genuine product or preference decision that evidence cannot settle.
...
- Shipping UI / IDE / CLI → use the available native computer/browser tools or an installed control skill. For bug fixes, reproduce first on the same surface when access permits.
...
- Long, autonomous, or multi-phase work, or any task the user steps away from to review later ("going to bed", "trust it when i'm back", "monitor until X") → a decision trail via the **show-me-your-work** skill. Commit it when stakes need an auditable record; keep it local otherwise.
...
**Stay within the request and current authorization.** Proceed with read-only inspection and reversible local implementation that the user requested. Do not infer permission for external messages, ticket changes, PR merges, deployments, purchases, destructive actions, or a material scope expansion.

**Pause for new authority.** Ask immediately before an external, destructive, costly, or otherwise consequential action unless the user already authorized that exact action.
...
- **Autonomous run.** A long task to drive to completion without stopping ("run until done", "monitor until X"). `playbooks/autonomous-run.md`.
