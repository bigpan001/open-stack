...
Mining misses intent that has not come up yet. Ask one or two short structured questions when the environment provides an input tool; otherwise ask concise free-text questions.
...
Keep the draft local by default. Commit or open a PR only when the user asked for repository delivery.
