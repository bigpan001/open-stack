...
Source control is available through local git when the target is a repository; `gh` is optional and may require authentication. For the other six, classify available MCPs using their names, tool descriptions, and resource descriptors. Record ambiguous cases in the coverage map.
...
Dispatch matching investigators concurrently with `spawn_agent`. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Do not ask one agent to cover multiple MCPs.

Subagent config (each):
- `task_name`: a short unique identifier
- `model`: your configured why-investigators model (default `gpt-5.6-luna`)
- Brief: read-only investigation. It may use available read-only MCPs but must not modify files or external state.
...
- `task_name`: a short unique identifier
- `model`: your configured why-synthesizer model (default `gpt-5.6-sol`)
- Brief: may use available read-only MCPs but must not write external state. The synthesizer's quality check spot-verifies citations, which can require MCP access. The brief must explicitly allow relevant read-only MCP lookups.
