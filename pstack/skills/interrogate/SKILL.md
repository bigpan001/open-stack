...
|----------|---------------|
| Reviewer A | strongest available judgment route |
| Reviewer B | strongest available balanced route |
| Reviewer C | strongest available economical route |

For each reviewer, use a unique `task_name`, a read-only brief, and the configured model plus reasoning effort only when the current host exposes both.

If a route is unavailable, omit the override and inherit the parent model. Record the fallback; do not mutate configuration or open a PR unless the user asks.
