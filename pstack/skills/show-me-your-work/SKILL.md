...
When the user authorized delegation and another concurrency slot is available, spawn a read-only reviewer, preferably on a different available model route. It reads the audit trail and current task evidence, then flags what the user should scrutinize. Otherwise perform the same audit locally and disclose that it was not an independent model pass.
