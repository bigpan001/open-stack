...
Use the current task history and Codex thread tools when available. Never scan broad transcript directories or unrelated tasks. If prior history is unavailable, say so and reconstruct state from the repository, branches, PRs, tickets, and the user's supplied context.
...
3. Fan out only when multiple in-scope task histories are available through Codex thread tools and the user asked to recall across them. Partition the selected tasks among read-only sub-agents. Each returns the same schema: topic, user goal, decisions, open threads, corrections, and artifacts. For one or two tasks, read them directly. Keep unrelated and private task histories out of scope.
