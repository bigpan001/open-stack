...
Start a new Codex task after installation so the plugin's skills are discovered. Invoke skills with `$skill-name`, for example `$poteto-mode` or `$interrogate`.
...
that's it. the other skills are situational; the mode skill uses them as needed. the Codex port inherits the parent model unless a configured role names a model exposed by the current host. [`$setup-pstack`](./skills/setup-pstack/SKILL.md) configures role-based model and reasoning-effort overrides.
...
[`$poteto-mode`](./skills/poteto-mode/SKILL.md) can use a Codex heartbeat automation when you explicitly ask for recurring monitoring or a future follow-up.
...
pstack also ships a subagent that runs my style end to end. spawn it from a parent agent via [a brief that explicitly invokes `$poteto-agent`](./agents/poteto-agent.md). it reads `poteto-mode` in full, including its inline principles index, before doing any work. substituting `generalPurpose` skips that read and drifts.

[`$poteto-mode`](./skills/poteto-mode/SKILL.md) and [a brief that explicitly invokes `$poteto-agent`](./agents/poteto-agent.md) route through the same wrapper.

pstack also ships [Comment Sicko](./agents/comment-sicko.md), a read-only comment reviewer available as a brief that explicitly invokes `$comment-sicko`. usually invoke it through [`$no-comments`](./skills/no-comments/SKILL.md), not directly.
